...
class KeyboardRecorder:
    def __init__(self, vehicle, camera_manager, hud, save_dir="_out"):
        ...
        self.throttle = 0.0
        self.brake = 0.0
        self.steer_inc = 0.05      # 每帧方向变化
        self.throttle_inc = 0.02   # 每帧加减速变化
        self.save_dir = save_dir
        os.makedirs(save_dir, exist_ok=True)
        self.recording = False
        self.control_file = os.path.join(save_dir, "control_data.csv")
        # CSV文件写入表头
    ...
    def update_control(self):
        ...
        # 更新方向盘
        if keys[pygame.K_a]:
            self.steer = max(-1.0, self.steer - self.steer_inc)
        elif keys[pygame.K_d]:
            self.steer = min(1.0, self.steer + self.steer_inc)
        else:
            # 自动回正
            self.steer *= 0.8
        ...
        # 更新油门/刹车
        if keys[pygame.K_w]:
            self.throttle = min(1.0, self.throttle + self.throttle_inc)
        else:
            self.throttle *= 0.9  # 松开油门慢慢减速

        if keys[pygame.K_s]:
            self.brake = min(1.0, self.brake + self.throttle_inc)
        else:
            self.brake *= 0.9
        ...
        # 保存图像+控制值
        if self.recording and self.camera_manager.index is not None:
            frame = self.camera_manager.sensor.get_transform().location.x  # 用x做frame标识
            image_path = os.path.join(self.save_dir, f"{frame:08d}.png")
            # 保存图像
            self.camera_manager.sensor.save_to_disk(image_path)
            # 保存控制值
            with open(self.control_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([frame, self.steer, self.throttle, self.brake])
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            recorder = KeyboardRecorder(world.player, world.camera_manager, hud)
            ...
            while True:
                ...
                if recorder.parse_events():
                    break

                recorder.update_control()  # 用键盘控制 + 记录数据
