...
max_action       = [1.0, 1.0 ,1.0]    ## [ max_steer , max_throttle, max_brake ]

## State Setting
state_class = 'lg'              ## 'g': always goal ; 'lg': serial intermidiate points & goal  ; 'glg': serial intermidiate points + goal
#inter_point_size = 2            ## mid point(x,y)
current_state_size = 3          ## currentx,currenty,currentyaw
goal_size        = 2            ## targetx,targety
frame_size       = 4            ## continuous frames
#waypoints_look_ahead_size = 2*3 ##3*(x,y)
boundary_size = 4               ##lx,ly,rx,ry
filtered_obstacles_size =4*5    ## 5*(ox,oy,vx,vy)
current_speed_size=1            ##speed


## State Length
state_size = goal_size + action_size + current_state_size + boundary_size + filtered_obstacles_size + current_speed_size
